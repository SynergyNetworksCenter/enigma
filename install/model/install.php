...
class ModelInstall extends Model {
	public function database($data) {
		...
		$file = DIR_APPLICATION . 'synergycms.sql';
		...
		if ($lines) {
			...
			foreach($lines as $line) {
				if ($line && (substr($line, 0, 2) != '--') && (substr($line, 0, 1) != '#')) {
					...
					if (preg_match('/;\s*$/', $line)) {
						$sql = str_replace("DROP TABLE IF EXISTS `sn_", "DROP TABLE IF EXISTS `" . $data['db_prefix'], $sql);
						$sql = str_replace("CREATE TABLE `sn_", "CREATE TABLE `" . $data['db_prefix'], $sql);
						$sql = str_replace("INSERT INTO `sn_", "INSERT INTO `" . $data['db_prefix'], $sql);
			...
			$db->query("INSERT INTO `" . $data['db_prefix'] . "user` 
				SET user_id = '1', 
				user_group_id = '1', 
			...
			$db->query("UPDATE `" . $data['db_prefix'] . "post` SET `viewed` = '0'");
		}		
	}	
}
?>