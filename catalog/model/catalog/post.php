...
class ModelCatalogPost extends Model {
	...
	public function getPostFeatures($post_id) {
		$query = $this->db->query("SELECT
			ad.name, pa.text
			FROM sn_post_attribute pa
			LEFT JOIN sn_attribute_description ad ON (pa.attribute_id = ad.attribute_id)
		...
		foreach ($query->rows as $post_attribute) {
			$attributes[$post_attribute['name']] = $post_attribute['text'];
		}
	...
	public function getPostFeatureAttributes($post_id) {
		...
		foreach ($post_attribute_group_query->rows as $post_attribute_group) {
			...
			foreach ($post_attribute_query->rows as $post_attribute) {
				$post_attribute_data[] = array(
					...
					$post_attribute['name']  => $post_attribute['text']
				);
			}
	...
	public function getPostOptionAttributes($post_id) {
		$post_attribute_group_data = array();
	...
	public function getPostAllowanceAttributes($post_id) {
		$post_attribute_group_data = array();
	...
	public function getDownload($post_id) {
		$query = $this->db->query(
			"SELECT d.filename 
			From sn_download d 
			Left Join sn_post_to_download pd on (d.download_id = pd.download_id)
			Where pd.post_id = '" . (int)$post_id . "'");

		return $query->row;
	}

	public function getDownloads($post_id) {
		$post_downloads = array();

		$query = $this->db->query("SELECT d.filename, d.mask 
			From sn_download d 
			Left Join sn_post_to_download pd on (d.download_id = pd.download_id)
			Where pd.post_id = '" . (int)$post_id . "'");
