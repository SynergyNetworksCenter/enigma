	...
	public function index() {
		...
		if ($post_info) {
			...
			if ($post_info['image']) {
				...
				$this->data['popup'] =  $post_info['image'];
			} else {
				$this->data['popup'] = '';
			}

			if ($post_info['image']) {
				// $this->data['thumb'] = $this->model_tool_image->resize($post_info['image'], $this->config->get('config_image_thumb_width'), $this->config->get('config_image_thumb_height'), 'h');
				$this->data['thumb'] =  $post_info['image'];
			} else {
				$this->data['thumb'] = '';
			}
			...
			$this->data['attribute_features'] = $this->model_catalog_post->getPostFeatureAttributes($this->request->get['post_id']);
			$this->data['attribute_options'] = $this->model_catalog_post->getPostOptionAttributes($this->request->get['post_id']);
			$this->data['attribute_allowances'] = $this->model_catalog_post->getPostAllowanceAttributes($this->request->get['post_id']);

			$this->data['downloads'] = array();

			$downloads = $this->model_catalog_post->getDownloads($this->request->get['post_id']);
			
			foreach ($downloads as $download ) {
				$this->data['downloads'][] = array(
					'name' => $download['name'],
					'href' => HTTP_SERVER . 'download/' . $download['filename']
				);
			}	
