...
class ModelCatalogPost extends Model {
	...
	public function enablePost($post_id) {
		$post_info = $this->getPost($post_id);
		
		if ($post_id) {
			$this->db->query("UPDATE " . DB_PREFIX . "post SET status = '1' WHERE post_id = '" . (int)$post_id . "'");
		}

		$this->cache->delete('post');
	}

	public function disablePost($post_id) {
		$post_info = $this->getPost($post_id);
		
		if ($post_id) {
			$this->db->query("UPDATE " . DB_PREFIX . "post SET status = '0' WHERE post_id = '" . (int)$post_id . "'");
		}
	...
	public function copyPost($post_id) {
		...
		if ($query->num_rows) {
			...
			$data = array_merge($data, array('post_site' => $this->getPostSites($post_id)));
	...
	public function getPostSite($post_id) {
		$query = $this->db->query("SELECT s.name as 'name'
			FROM sn_post_to_site ps
			LEFT JOIN sn_site s ON (ps.site_id = s.site_id)
			Where ps.post_id = $post_id");

		return $query->row['name'];
	}

	public function getPostCategoryName($post_id) {
		$query = $this->db->query("SELECT cd.name
			FROM sn_post_to_category pc 
			LEFT JOIN sn_category_description cd ON (pc.category_id = cd.category_id) 
