<?php 
class ControllerCatalogBing extends Controller {
	private $error = array();

	public function index() {
		$this->language->load('catalog/bing');
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_bing->addBing($this->request->post);

			$this->session->data['success'] = $this->language->get('text_success');

			$this->redirect($this->url->link('catalog/bing', 'token=' . $this->session->data['token'], 'SSL'));
		}
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_bing->editBing($this->request->get['id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $id) {
				$this->model_catalog_bing->deleteBing($id);
			}
	...
	protected function getList() {
		...
		//Set Url for Buttons
		$this->data['insert'] = $this->url->link('catalog/bing/insert', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['delete'] = $this->url->link('catalog/bing/delete', 'token=' . $this->session->data['token'], 'SSL');

		//Declares array to store the page data for view
		$this->data['bing'] = array();

		// Get Total Number of Websites 
		$bing_total = $this->model_catalog_bing->getTotalBing();

		// Get all website data to display
		$results = $this->model_catalog_bing->getBingAccounts();
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('catalog/bing/update', 'token=' . $this->session->data['token'] . '&id=' . $result['id'], 'SSL'),
				'class' => 'btn-edit'
			);
		...
		$this->data['column_bing_id'] = $this->language->get('column_bing_id');
		...
		$this->template = 'catalog/bing_list.tpl';
		$this->children = array(
			'common/header',
			'common/navigation',
			'common/footer'
	...
	protected function getForm() {
		...
		$this->data['entry_bing_id'] = $this->language->get('entry_bing_id');
		...
		if (isset($this->error['bing_id'])) {
			$this->data['error_bing_id'] = $this->error['bing_id'];
		} else {
			$this->data['error_bing_id'] = '';
		}
		...
		if (!isset($this->request->get['id'])) {
			$this->data['action'] = $this->url->link('catalog/bing/insert', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$this->data['action'] = $this->url->link('catalog/bing/update', 'token=' . $this->session->data['token'] . '&id=' . $this->request->get['id'], 'SSL');
		}

		$this->data['cancel'] = $this->url->link('catalog/bing', 'token=' . $this->session->data['token'], 'SSL');

		if (isset($this->request->get['id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$bing_info = $this->model_catalog_bing->getBingAccount($this->request->get['id']);
		}

		if (isset($this->request->post['title'])) {
			$this->data['title'] = $this->request->post['title'];
		} elseif (!empty($bing_info)) {
			$this->data['title'] = $bing_info['title'];
		} else {
			$this->data['title'] = '';
		}
		...
		if (isset($this->request->post['website_id'])) {
			...
			$this->data['website_id'] = $bing_info['website_id'];
		} else {
		...
		if (isset($this->request->post['website'])) {
			...
			$website_info = $this->model_catalog_website->getWebsite($bing_info['website_id']);
		...
		if (isset($this->request->post['bing_id'])) {
			...
			$this->data['bing_id'] = $bing_info['bing_id'];
		} else {
			$this->data['bing_id'] = '';
		}
		...
		if (isset($this->request->post['status'])) {
			...
			$this->data['status'] = $bing_info['status'];
		} else {
			$this->data['status'] = 0;
		}

		$this->data['token'] = $this->session->data['token'];
		$this->template = 'catalog/bing_form.tpl';
		$this->children = array(
			'common/header',
			'common/navigation',
			'common/footer'
	...
	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'catalog/bing')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}


		if(!(utf8_strlen($this->request->post['bing_id']) <= 0)) {

			if ((utf8_strlen($this->request->post['bing_id']) < 7) || (utf8_strlen($this->request->post['bing_id']) > 10)) {
				$this->error['bing_id'] = $this->language->get('error_bing_id');
			}
		} else {
			$this->error['bing_id'] = $this->language->get('error_bing_empty');
		}
