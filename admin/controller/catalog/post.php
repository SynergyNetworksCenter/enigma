...
class ControllerCatalogPost extends Controller {
		...
		if (!$this->user->hasPermission('modify', 'catalog/post')) {
			...
			foreach ($this->request->post['selected'] as $post_id) {
				...
				if ($post_info && !$post_info['status']) {
					$this->model_catalog_post->enablePost($post_id);
							  
					$enabled_count++;
				}
			} 
					  
			$this->session->data['success'] = sprintf($this->language->get('text_post_enabled'), $enabled_count);	
			...
			public function disable() {
				  ...
				  if (!$this->user->hasPermission('modify', 'catalog/post')) {
					  ...
					  foreach ($this->request->post['selected'] as $post_id) {
						  ...
						  if ($post_info && $post_info['status']) {
							  $this->model_catalog_post->disablePost($post_id);
							  
							  $disabled_count++;
						  }
					  } 
					  
					  $this->session->data['success'] = sprintf($this->language->get('text_post_disabled'), $disabled_count);	
	...
	protected function getList() {
		...
		$this->data['enable'] = $this->url->link('catalog/post/enable', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['disable'] = $this->url->link('catalog/post/disable', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			...
			$site_name = $this->model_catalog_post->getPostSite($result['post_id']);
			if ($site_name == 'Poseidon Homes') {
				$site = 'Poseidon German';
			} else {
				$site = 'Poseidon English';
			}

			$category_name = $this->model_catalog_post->getPostCategoryName($result['post_id']);
