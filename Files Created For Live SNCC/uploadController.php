...
class ControllerCatalogUpload extends Controller {
    ...
    $this->document->addStyle('view/js/vendor/chosen/css/chosen.min.css');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect($this->url->link('catalog/upload', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_upload->editUpload($this->request->get['upload_id'], $this->request->post);
	...
	protected function getList() {
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('catalog/upload', 'token=' . $this->session->data['token'] . $url, 'SSL'),
			'separator' => ' / '
		);

		$this->data['insert'] = $this->url->link('catalog/upload/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['delete'] = $this->url->link('catalog/upload/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('catalog/upload/update', 'token=' . $this->session->data['token'] . '&upload_id=' . $result['upload_id'] . $url, 'SSL')
			);
		...
		$this->data['sort_name'] = $this->url->link('catalog/upload', 'token=' . $this->session->data['token'] . '&sort=dd.name' . $url, 'SSL');
		$this->data['sort_remaining'] = $this->url->link('catalog/upload', 'token=' . $this->session->data['token'] . '&sort=d.remaining' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/upload', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['upload_id'])) {
			$this->data['action'] = $this->url->link('catalog/upload/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('catalog/upload/update', 'token=' . $this->session->data['token'] . '&upload_id=' . $this->request->get['upload_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('catalog/upload', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		if (isset($this->request->post['filename'])) {
			...
			$this->data['filename'] = $upload_info['filename'];
		} else {
			$this->data['filename'] = '';
		}
		...
		if (isset($this->request->post['mask'])) {
			...
			$this->data['mask'] = $upload_info['mask'];
		} else {
			$this->data['mask'] = '';
		}
		...
		if (isset($this->request->post['remaining'])) {
			...
			$this->data['remaining'] = $upload_info['remaining'];
		} else {
	...
	protected function validateForm() {
		...
		if (!file_exists(DIR_UPLOAD . $this->request->post['filename']) && !is_file(DIR_UPLOAD . $this->request->post['filename'])) {
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $upload_id) {
			$post_total = $this->model_catalog_post->getTotalPostsByUploadId($upload_id);
